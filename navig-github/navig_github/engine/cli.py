"""
The GitHub engine CLI interface.
...
app = typer.Typer(
    name="github",
    help="🥔 The navig-github engine — GitHub backup, mirror, export & manage.",
...
def version_callback(value: bool) -> None:
    ...
    if value:
        ...
        console.print(f"The GitHub engine version {__version__}")
        console.print("Repository: https://github.com/navig-run/core")
        raise typer.Exit()
...
def user(
    ...
    Example:
        navig github user miztizm
        navig github user miztizm --dest ./custom_backups
        navig github user miztizm --include-issues --include-pulls --include-wikis
        navig github user miztizm --bare --lfs  # Mirror clone with LFS support
        navig github user miztizm --exclude repo1 --exclude repo2  # Exclude specific repos
        navig github user miztizm --name-regex '^my-prefix-.*'  # Only repos matching pattern
        navig github user miztizm --incremental  # Only repos changed since last backup
    """
    # Use default destination if not provided
    if dest is None:
        dest = get_default_user_dest(username)

    if token and token == typer.get_app_dir("navig-github"):
...
def org(
    ...
    Example:
        navig github org github
        navig github org github --dest ./custom_backups
        navig github org myorg --include-issues --include-pulls --include-wikis
        navig github org myorg --bare --lfs  # Mirror clone with LFS support
        navig github org myorg --name-regex '^project-.*'  # Only repos matching pattern
        navig github org myorg --incremental  # Only repos changed since last backup
    """
    # Use default destination if not provided
    if dest is None:
...
def profile(
    ...
    Example:
        ...
        navig github profile miztizm
        navig github profile miztizm --dest ./custom_profile.yaml --format yaml
    """
...
def starred(
    ...
    Example:
        ...
        navig github starred miztizm
        navig github starred miztizm --dest ./custom_starred
    """
...
def watched(
    ...
    Example:
        ...
        navig github watched miztizm
        navig github watched miztizm --dest ./custom_watched
    """
    # Need to get the actual username first to determine default dest
    temp_config = Config(
        target_type=TargetType.USER,
...
def secrets(
    ...
    Example:
        navig github secrets miztizm/navig-github
        navig github secrets miztizm/hello-world --format yaml
        navig github secrets miztizm/navig github --dest ./custom_secrets.json
    """
    if format.lower() not in ["json", "yaml"]:
        console.print("[red]❌ Error: Format must be 'json' or 'yaml'[/red]")
        sys.exit(1)
...
def delete(
    ...
    Example:
        navig github delete miztizm/testdelete
        navig github delete miztizm/testdelete --force
    """
    # Parse owner/repo
    parts = repository.split("/")
    if len(parts) != 2:
...
def repo(
    ...
    Example:
        navig github repo microsoft/vscode
        navig github repo miztizm/hello-world --include-issues --include-pulls
        navig github repo myorg/myrepo --all
        navig github repo python/cpython --dest ./my-backups --include-wikis
    """
    # Parse owner/repo
    parts = repository.split("/")
    if len(parts) != 2:
...
def clone(
    ...
    Unlike `navig github repo` (which builds a structured backup tree), `clone`
    ...
    "Sometimes you just want the code, not the whole archive." — navig github

    Example:
        navig github clone microsoft/vscode
        navig github clone https://github.com/python/cpython cpython-src
        navig github clone myorg/myrepo --use-ssh
        navig github clone myorg/myrepo --force   # re-sync an existing checkout
    """
...
def issues(
    ...
    Example:
        navig github issues miztizm/hello-world
        navig github issues miztizm/navig github --state open --include-comments
        navig github issues miztizm/navig github --dest ./my-issues.json --format yaml
    """
    # Parse owner/repo
    parts = repository.split("/")
    if len(parts) != 2:
...
def pulls(
    ...
    Example:
        navig github pulls miztizm/hello-world
        navig github pulls miztizm/navig github --state open --include-comments
        navig github pulls miztizm/navig github --dest ./my-prs.json --format yaml
    """
    # Parse owner/repo
    parts = repository.split("/")
    if len(parts) != 2:
...
def workflows(
    ...
    Example:
        navig github workflows miztizm/hello-world
        navig github workflows miztizm/navig github --include-runs
        navig github workflows miztizm/navig github --dest ./my-workflows/
    """
    # Parse owner/repo
    parts = repository.split("/")
    if len(parts) != 2:
...
def releases(
    ...
    Example:
        navig github releases miztizm/hello-world
        navig github releases miztizm/navig github --download-assets
        navig github releases miztizm/navig github --dest ./my-releases/
    """
    # Parse owner/repo
    parts = repository.split("/")
    if len(parts) != 2:
...
def wiki(
    ...
    Example:
        navig github wiki miztizm/hello-world
        navig github wiki miztizm/navig github --dest ./my-wiki/
    """
    # Parse owner/repo
    parts = repository.split("/")
    if len(parts) != 2:
...
def search(
    ...
    Examples:
        ...
        navig github search "smsbomber" --limit 10

        # Search with multiple filters
        navig github search "machine learning" --language python --min-stars 1000 --limit 20

        # Search and auto-confirm (skip prompt)
        navig github search "react components" --limit 5 --yes

        # Search with custom output directory
        navig github search "awesome-python" --output-dir ./my-collections --limit 15

        # Search with flat structure (no owner subdirectories)
        navig github search "cli tools" --flat-structure --limit 10
    """
...
def gists(
    ...
    Example:
        navig github gists
        navig github gists miztizm
        navig github gists miztizm --starred  # Include starred gists
        navig github gists --dest ./my-gists --skip-existing
    """
...
def attachments(
    ...
    Example:
        navig github attachments miztizm/navig-github
        navig github attachments myorg/myrepo --source issues
        navig github attachments myorg/myrepo --source pulls --dest ./my-attachments
    """
...
def labels(
    ...
    Example:
        navig github labels miztizm/navig-github
        navig github labels myorg/myrepo --format yaml
        navig github labels myorg/myrepo --dest ./my-labels.json
    """
    if format.lower() not in ["json", "yaml"]:
        console.print("[red]❌ Error: Format must be 'json' or 'yaml'[/red]")
        sys.exit(1)
...
def milestones(
    ...
    Example:
        navig github milestones miztizm/navig-github
        navig github milestones myorg/myrepo --state open
        navig github milestones myorg/myrepo --format yaml
    """
    if format.lower() not in ["json", "yaml"]:
        console.print("[red]❌ Error: Format must be 'json' or 'yaml'[/red]")
        sys.exit(1)
...
def webhooks(
    ...
    Example:
        navig github webhooks miztizm/navig-github
        navig github webhooks myorg/myrepo --format yaml
    """
    if format.lower() not in ["json", "yaml"]:
        console.print("[red]❌ Error: Format must be 'json' or 'yaml'[/red]")
        sys.exit(1)
...
def followers(
    ...
    Example:
        ...
        navig github followers miztizm
        navig github followers miztizm --include-following
        navig github followers --format yaml
    """
    if format.lower() not in ["json", "yaml"]:
        console.print("[red]❌ Error: Format must be 'json' or 'yaml'[/red]")
        sys.exit(1)
...
def discussions(
    ...
    Example:
        navig github discussions miztizm/navig-github
        navig github discussions myorg/myrepo --format yaml
    """
    if format.lower() not in ["json", "yaml"]:
        console.print("[red]❌ Error: Format must be 'json' or 'yaml'[/red]")
        sys.exit(1)
...
def config_save(
    ...
    Example:
        navig github config-save my-backup --type user --name miztizm --include-issues
        navig github config-save daily-org --type org --name myorg --workers 8
    """
...
def config_load(
    ...
    Example:
        navig github config-load my-backup
    """
...
def config_list() -> None:
    ...
    if not profiles:
        ...
        console.print("   [dim]Use 'navig github config-save' to create a profile[/dim]")
        return
...
def config_delete(
    ...
    Example:
        navig github config-delete my-backup
        navig github config-delete old-profile --force
    """
    from .config import ConfigManager
...
def config_export(
    ...
    Example:
        navig github config-export my-backup --output ./my-backup.yaml
    """
    from .config import ConfigManager
...
def config_import(
    ...
    Example:
        navig github config-import ./shared-backup.yaml
        navig github config-import ./backup.yaml --name my-new-profile
    """
    from .config import ConfigManager
...
def verify_backup(
    ...
    Example:
        navig github verify ./backups/miztizm
        navig github verify ./backups/miztizm/repos/public/miztizm/navig github --deep
        navig github verify ./my-backup --deep --checksums
    """
...
def schedule_add(
    ...
    Example:
        navig github schedule-add daily-backup --profile my-backup --interval daily --at 02:00
        navig github schedule-add weekly-full --profile full-backup --interval weekly --on monday --at 03:00
        navig github schedule-add frequent --profile quick --interval "every 6 hours"
    """
    from .scheduler import BackupScheduler, ScheduleStoreError, create_scheduled_backup
    ...
    try:
        ...
        print_error(f"Could not read existing schedules — not saving, to avoid losing them: {e}")
        console.print("   [dim]The schedule store is temporarily locked; try again in a moment.[/dim]")
        sys.exit(1)
...
def schedule_list() -> None:
    ...
    if not backups:
        ...
        console.print("   [dim]Use 'navig github schedule-add' to create a schedule[/dim]")
        return
...
def schedule_remove(
    ...
    Example:
        navig github schedule-remove daily-backup
    """
    from .scheduler import BackupScheduler, ScheduleStoreError

    scheduler = BackupScheduler()
    try:
        removed = scheduler.remove_backup(name)
    except ScheduleStoreError as e:
        print_error(f"Could not read existing schedules — not saving, to avoid losing them: {e}")
        console.print("   [dim]The schedule store is temporarily locked; try again in a moment.[/dim]")
        sys.exit(1)
...
def schedule_run(
    ...
    Example:
        ...
        navig github schedule-run --once  # Run pending jobs and exit
    """
    from .config import ConfigManager
    from .scheduler import BackupScheduler
...
def restore_issues(
    ...
    Example:
        navig github restore-issues ./backup/issues.json --to miztizm/new-repo
        navig github restore-issues ./issues.json --to myorg/myrepo --dry-run
    """
...
def restore_releases(
    ...
    Example:
        navig github restore-releases ./backup/releases/ --to miztizm/new-repo
        navig github restore-releases ./metadata.json --to myorg/myrepo --dry-run
    """
    from .restore import RestoreManager
...
def restore_labels(
    ...
    Example:
        navig github restore-labels ./backup/labels.json --to miztizm/new-repo
        navig github restore-labels ./labels.json --to myorg/myrepo --dry-run
    """
    from .restore import RestoreManager
...
def restore_milestones(
    ...
    Example:
        navig github restore-milestones ./backup/milestones.json --to miztizm/new-repo
        navig github restore-milestones ./milestones.json --to myorg/myrepo --dry-run
    """
    from .restore import RestoreManager
...
def projects(
    ...
    Example:
        navig github projects miztizm  # User/org projects
        navig github projects miztizm/navig github  # Repository projects
        navig github projects myorg/myrepo --format yaml
    """
    if format.lower() not in ["json", "yaml"]:
        console.print("[red]❌ Error: Format must be 'json' or 'yaml'[/red]")
        sys.exit(1)
...
def analytics_report(
    ...
    Example:
        ...
        navig github analytics ./backups/miztizm
        navig github analytics --format json --output report.json
    """
...
def analytics_history(
    ...
    Example:
        ...
        navig github analytics-history ./backups/miztizm --limit 50
    """
    from .analytics import BackupAnalytics
...
def diff_backups(
    ...
    Example:
        navig github diff ./backups-old ./backups-new
        navig github diff ./backups  # Compare with last snapshot
        navig github diff ./backup1 ./backup2 --format json
    """
    ...
    if new_path:
        ...
        if diff is None:
            print_warning("No snapshot found. Create one first with 'navig github snapshot'")
            sys.exit(1)
...
def create_snapshot(
    ...
    Example:
        navig github snapshot ./backups/miztizm
    """
    ...
    console.print("   [dim]Use 'navig github diff' to compare future changes[/dim]")
...
def list_templates(
    ...
    Example:
        ...
        navig github templates --category org
        navig github templates --tag scheduled
    """
    ...
    console.print("   [dim]Use 'navig github template-show <id>' for details[/dim]")
...
def show_template(
    ...
    Example:
        navig github template-show user-complete
        navig github template-show org-compliance
    """
    from .templates import TemplateManager
    ...
    if template.schedule_interval:
        # Rich parses each print() independently, so a style opened in one call does not
        # carry into the next. Splitting `[cyan]…[/cyan]` across two prints left the
        # second holding a closing tag with no opener, and Rich raises MarkupError on
        # that rather than ignoring it — `template-show` crashed for any template with
        # both an interval and a time. Each line now opens and closes its own tag.
        console.print(f"\n   [cyan]📅 Schedule: {template.schedule_interval}[/cyan]")
        if template.schedule_time:
            console.print(f"      [cyan]At: {template.schedule_time}[/cyan]")
...
def use_template(
    ...
    Example:
        navig github template-use user-complete miztizm
        navig github template-use org-compliance myorg --dest ./org-backup
    """
    from .templates import TemplateManager
...
def create_template(
    ...
    Example:
        navig github template-create my-template --name "My Template" --from-profile daily-backup
    """
    ...
    console.print("   [dim]Use 'navig github template-show' to view details[/dim]")
...
def delete_template(
    ...
    Example:
        navig github template-delete my-template
    """
    from .templates import TemplateManager
...
def test_notifications() -> None:
    ...
    if not manager.providers:
        ...
        console.print("   [dim]Configure notifications in ~/.config/navig-github/.navig-github_notifications.json[/dim]")
        return
...
def transfer(
    ...
    Example:
        navig github transfer my-repo --org my-org
        navig github transfer repo1,repo2,repo3 --org my-org
        navig github transfer @repos.txt --org my-org
        navig github transfer my-repo --org my-org --new-name new-repo-name
        navig github transfer my-repo --org my-org --dry-run
    """
