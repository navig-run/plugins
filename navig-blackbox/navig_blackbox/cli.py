"""Standalone entry point for the ``navig-blackbox`` and ``nbb`` console scripts.

Installs and runs without navig — it invokes the same ``blackbox_app`` Typer app that,
inside navig, backs the ``navig blackbox`` verb (via the core shim).
"""
from __future__ import annotations

import sys

from navig_blackbox.commands.blackbox import blackbox_app
...
def _ensure_utf8_stdio() -> None:
    ...
    Measured on cp1251 against the PUBLISHED wheel: `nbb --help` tracebacks, and the
