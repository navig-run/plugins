...
`dedupe_app`'s Typer help contains emoji. On a console whose encoding is an ANSI/OEM code page
...
against the PUBLISHED wheel on cp1251: `ndup --help` tracebacked; the same command under
PYTHONUTF8=1 worked.
...
def _run(encoding: str) -> subprocess.CompletedProcess:
    return subprocess.run(
        [sys.executable, "-c", "from navig_dedupe.cli import main; main()", "--help"],
        capture_output=True,
        text=True,
        encoding="utf-8",
        errors="replace",
...
def test_help_survives_a_legacy_console(encoding: str) -> None:
    ...
    assert "UnicodeEncodeError" not in (r.stderr or ""), (
        f"`ndup --help` crashed under PYTHONIOENCODING={encoding}:\n{r.stderr[-800:]}"
    )
...
def test_the_guard_runs_before_the_app() -> None:
    ...
    from navig_dedupe import cli

    src = inspect.getsource(cli.main)
    assert "_ensure_utf8_stdio()" in src, "the CLI no longer reconfigures its stdio"
    assert src.index("_ensure_utf8_stdio()") < src.index("dedupe_app()"), (
